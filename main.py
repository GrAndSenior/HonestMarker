...
import os, sys, pymysql #,pyodbc , mysql.connector
import pandas as pd
import warnings
...
class MarkApplication(QMainWindow, design.Ui_MainWindow):
    def __init__(self, parent):
        ...
        self.setWindowTitle('Маркировка Datamatrix - HonestMarker v2.1')
...
class LoginWindow(QMainWindow, loginwindow.Ui_LoginWindow):
    ...
    def __init__(self):
        ...
        strong = 'btsbrest'
        self.key = f"{strong}{self.cfg.get('MAIN','key')[::-1]}"   # загрузка ключа из внешнего источника
    ...
    def setup_action(self):
        ...
        self.cbLocalConnection.toggled.connect(self.sqlConnect)
        
    def sign_check(self, login, password):
        self.userid = -1
        if self.cbLocalConnection.isChecked():        
            return self.leLogin.text().lower() == 'test' and self.lePassword.text().lower() == 'test'
        else:
            ...
            result = None
            self.cursor.execute(f"SELECT id_manager, STATE, PASSWORD FROM MANAGER WHERE LOGIN = '{login}';")
    ...
    def sqlConnect(self):
        try:
            if not self.cbLocalConnection.isChecked():
                #print('DB connect...')
                ...
                if os.path.exists(os.path.abspath('sources/local.db')):
                    self.mydb = sqlite3.connect('sources/local.db')
                    ...
                    # Создаем таблицу Users для локального использования
                    self.cursor.execute('''
                    CREATE TABLE USERS (
                    ...
                    ''')
                    self.cursor.execute(''' 
                        INSERT INTO Users (NAME, LOGIN, PASSWORD, STATE, LEVEL, PROFILE) 
                        VALUES (?, ?, ?, ?, ?, ?)''', ('default', 'test', 'test', 2, 0, 'default'))

                    # Сохраняем изменения и закрываем соединение
                    self.mydb.commit()
                    #print('sqlite3 create...')
                self.statusBar().showMessage(f'Соединение с локальным сервером MySQL установлено', 3000)
                return True
