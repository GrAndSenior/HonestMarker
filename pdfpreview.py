...
class AutoCropThread(QThread):
    ...
    def run(self):
        ...
        pdfIn = fitz.open(self.parent.fileName)
...
class PdfPreview(QMainWindow, pdfimport.Ui_PdfPreviewWindow):
    def __init__(self, parent, fileName, **kwargs) -> None:
        ...
        self.leFileName.setVisible(False)
        ...
        if self.parent.tabWidget.currentIndex() == 0:
            ...
            self.data_frame = pd.DataFrame(columns=['K'])
        self.accept_actions()
        self.on_openfile()        
    ...
    def on_preview_click(self):
        ...
        if self.gbDmGrid.isChecked():
            try:
                ...
                self.statusBar().showMessage(f'Ошибка распознавания кода', 0)
        else:
            img = np.array(img).copy()
    ...
    def on_openfile(self):
        ...
        try:
            ...
            self.statusBar().showMessage(f'Ошибка при попытке обработать файл {self.fileName}', 0)

    def dm_decode(self, np_image, point, size):
        try:
            image = Image.fromarray(np_image[point[0]-10:point[0]+size[1]+10, point[1]-10: point[1]+size[0]+10].copy())
            image.show()
